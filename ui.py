...
CamXCoor = InputField(model='quad', text='x: ', character_limit=6, max_lines=1, origin=(-.5, .5), position=(.05, -.3), scale_x=.25, scale_y=.5, parent=CamCoorGroup)
CamYCoor = InputField(model='quad', text='y: ', character_limit=6, max_lines=1, origin=(-.5, .5), position=(CamXCoor.x + CamXCoor.scale_x + .05, CamXCoor.y), scale_x=CamXCoor.scale_x, scale_y=CamXCoor.scale_y, parent=CamCoorGroup)
CamZCoor = InputField(model='quad', text='z: ', character_limit=6, max_lines=1, origin=(-.5, .5), position=(CamYCoor.x + CamYCoor.scale_x + .05, CamYCoor.y), scale_x=CamYCoor.scale_x, scale_y=CamYCoor.scale_y, parent=CamCoorGroup)
